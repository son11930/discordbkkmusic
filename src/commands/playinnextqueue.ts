...
export const data = new SlashCommandBuilder()
  .setName('playinnextqueue')
  .setDescription('Add a song to play immediately after the current one')
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const result = await player.addNextAndPlay(songInfo);
    if (result === 'playing') {
      return interaction.editReply(`🎶 Now playing: **${songInfo.title}**`);
    } else if (result === 'queued') {
      return interaction.editReply(`📝 Added as NEXT in queue: **${songInfo.title}**`);
    } else {
      return interaction.editReply(`❌ Failed to play **${songInfo.title}**.`);
    }
  } catch (error) {
    console.error(error);
